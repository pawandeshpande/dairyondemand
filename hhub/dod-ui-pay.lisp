...
(defun dod-controller-make-payment-request-html ()
  (let* ((description "This is test description")
	 ...
	 (param-values (list amount *PAYMENTAPIKEY* customer-city customer-country currency description customer-email mode  customer-name order-id  customer-phone return-url show-convenience-fee return-url-cancel return-url-failure udf1 udf2 udf3 udf4 udf5  customer-zipcode))
...
(defun dod-controller-customer-payment-successful-page ()
 (let* ((transaction-id (hunchentoot:parameter "transaction_id"))
	...
	(response-code (parse-integer (hunchentoot:parameter "response_code")))
	...
	(calculated-hash (generatehashkey   params-alist  *PAYMENTAPISALT* :sha512)))
	
					;Update customer's wallet first
       (when (and (equal response-code 0)
		  (responsehashcheck postparams  *PAYMENTAPISALT* :sha512))
